...
/**
 * An API module of Hannibal Finance contracts.
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    try {
      ...
      console.error(`getDefaultProvider() error: ${e.stack}`);
    }
    ...
    for (const [name, deployment] of Object.entries(deployments)) {
      try {
        ...
        console.error(`this.contracts[name:${name}] = new Contract(deployment.address:${deployment.address}, deployment.abi, provider:${provider}) error: ${e.stack}`);
      }
    }
    ...
    for (const [symbol, [address, decimal]] of Object.entries(externalTokens)) {
      try {
        ...
        console.error(`this.externalTokens[symbol:${symbol}] = new ERC20(address:${address}, provider${provider}, symbol${symbol}, decimal${decimal}) error: ${e.stack}`);
      }
    }

    try {
      this.Han = new ERC20(deployments.Han.address, provider, 'HAN');
    } catch (e) {
      console.error(`ERC20(deployments.Han.address, provider, 'HAN') error: ${e.stack}`);
    }

    try {
      this.HaSHARE = new ERC20(deployments.HaShare.address, provider, 'HASHARE');
    } catch (e) {
      console.error(`ERC20(deployments.HaShare.address, provider, 'HASHARE') error: ${e.stack}`);
    }

    try {
      this.HaBOND = new ERC20(deployments.HaBond.address, provider, 'HABOND');
    } catch (e) {
      console.error(`ERC20(deployments.HaBond.address, provider, 'HABOND') error: ${e.stack}`);
    }
    ...
    try {
      ...
      console.error(`this.FTM = this.externalTokens['WFTM'] error: ${e.stack}`);
    }

    // Uniswap V2 Pair
    try {
      this.TOMBWFTM_LP = new Contract(externalTokens['HAN-FTM-LP'][0], IUniswapV2PairABI, provider);
    } catch (e) {
      console.error(`this.TOMBWFTM_LP = new Contract(externalTokens['HAN-FTM-LP'][0], IUniswapV2PairABI, provider) error: ${e.stack}`);
    }
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.Han, this.HaSHARE, this.HaBOND, ...Object.values(this.externalTokens)];
  ...
  async getTombStat(): Promise<TokenStat> {
    const { HanFtmRewardPool, HanFtmLpHanRewardPool, HanFtmLpHanRewardPoolOld } = this.contracts;
    const supply = await this.Han.totalSupply();
    const tombRewardPoolSupply = await this.Han.balanceOf(HanFtmRewardPool.address);
    const tombRewardPoolSupply2 = await this.Han.balanceOf(HanFtmLpHanRewardPool.address);
    const tombRewardPoolSupplyOld = await this.Han.balanceOf(HanFtmLpHanRewardPoolOld.address);
    const tombCirculatingSupply = supply
      .sub(tombRewardPoolSupply)
      .sub(tombRewardPoolSupply2)
      .sub(tombRewardPoolSupplyOld);
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.Han);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.Han.decimal, 0),
      circulatingSupply: getDisplayBalance(tombCirculatingSupply, this.Han.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('HAN') ? this.Han : this.HaSHARE;
    const isTomb = name.startsWith('HAN');
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const supply = await this.HaBOND.displayedTotalSupply();
    return {
  ...
  async getShareStat(): Promise<TokenStat> {
    const { HanFtmLPHaShareRewardPool } = this.contracts;

    const supply = await this.HaSHARE.totalSupply();

    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.HaSHARE);
    const tombRewardPoolSupply = await this.HaSHARE.balanceOf(HanFtmLPHaShareRewardPool.address);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.HaSHARE.decimal, 0),
      circulatingSupply: getDisplayBalance(tShareCirculatingSupply, this.HaSHARE.decimal, 0),
    };
  }

  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, HanFtmRewardPool } = this.contracts;
    const expectedPrice = await SeigniorageOracle.twap(this.Han.address, ethers.utils.parseEther('1'));
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'HAN' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (depositTokenName.startsWith('HAN')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WFTM') {
      ...
      if (tokenName === 'HAN-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.Han, true);
      } else if (tokenName === 'TSHARE-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.HaSHARE, false);
  ...
  async getTotalValueLocked(): Promise<Number> {
    ...
    const masonrytShareBalanceOf = await this.HaSHARE.balanceOf(this.currentMasonry().address);
    const masonryTVL = Number(getDisplayBalance(masonrytShareBalanceOf, this.HaSHARE.decimal)) * Number(TSHAREPrice);
  ...
  async getMasonryAPR() {
    ...
    const masonrytShareBalanceOf = await this.HaSHARE.balanceOf(Masonry.address);
    const masonryTVL = Number(getDisplayBalance(masonrytShareBalanceOf, this.HaSHARE.decimal)) * Number(TSHAREPrice);
  ...
  async canUserUnstakeFromMasonry(): Promise<boolean> {
    ...
    const notStaked = Number(getDisplayBalance(stakedAmount, this.HaSHARE.decimal)) === 0;
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === FTM_TICKER) {
      ...
      const token = tokenName === TOMB_TICKER ? this.Han : this.HaSHARE;
