...
░▓█▒░██▓ ▓█   ▓██▒▒██░   ▓██░▒██░   ▓██░░██░░▓█  ▀█▓ ▓█   ▓██▒░██████▒   ░▒█░    ░██░▒██░   ▓██░ ▓█   ▓██▒▒██░   ▓██░▒ ▓███▀ ░░▒████▒
 ...
 ░  ░  ░      ░  ░         ░          ░  ░   ░            ░  ░    ░  ░            ░           ░       ░  ░         ░ ░ ░         ░  ░
    ...
    http://han.finance
*/
contract Han is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 HAN-WFTM LP -> HAN pool
    uint256 public constant INITIAL_HAN_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the HAN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HAN", "HAN") {
        // Mints 1 HAN to contract creator for initial pool setup
    ...
    function _getHanPrice() internal view returns (uint256 _hanPrice) {
        try IOracle(hanOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Han: failed to fetch HAN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _hanPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_hanPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHanOracle(address _hanOracle) public onlyOperatorOrTaxOffice {
        require(_hanOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints HAN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HAN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHanPrice = _getHanPrice();
            currentTaxRate = _updateTaxRate(currentHanPrice);
            if (currentHanPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_hanPool != address(0), "!_hanPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_hanPool, INITIAL_HAN_POOL_DISTRIBUTION);
